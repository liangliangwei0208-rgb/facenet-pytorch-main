...
这个项目里的 FaceNet 更准确地说是在做人脸验证：它不直接输出“这个人是谁”，而是判断两张人脸图片是否足够相似。模型会先把每张人脸图片变成一个 128 维特征向量，也可以理解成“这张脸在特征空间里的坐标”，然后比较两个坐标之间的距离。
...
输入人脸图片
    |
    v
resize 到 160x160，并做归一化预处理
    |
    v
mobilenet 或 inception_resnetv1 提取人脸特征
    |
    v
全连接层得到 128 维 embedding
    |
    v
L2 normalize，把特征向量归一到单位长度
    |
    v
计算两张图片 embedding 的欧氏距离
    |
    v
距离小：更像同一个人；距离大：更像不同人
```

模型可以看成一个函数 `f(x)`，输入图片 `x`，输出 128 维向量：

```text
图片1 -> f(x1) = [0.12, -0.08, ..., 0.03]
图片2 -> f(x2) = [0.10, -0.06, ..., 0.01]

d = ||f(x1) - f(x2)||_2
```

这里的 `d` 就是两张图片的距离。`facenet.py` 里的 `detect_image()` 使用 `np.linalg.norm(output1 - output2, axis=1)` 计算这个距离。距离越小，说明两个 128 维坐标越接近，两张脸越相似；距离越大，说明坐标分得更开，两张脸差异更明显。

为了方便直观看，可以把真实的 128 维特征空间简化成二维平面：
...
A1、A2、A3 表示同一个人的不同照片，点会尽量聚在一起。
B1、B2 表示另一个人的照片，会和 A 这一组保持距离。
```

训练时，`nets/facenet_training.py` 中的 `triplet_loss(alpha=0.2)` 会让模型学习这种分布关系：同一个人的 `anchor` 和 `positive` 距离要更近，不同人的 `negative` 距离要更远，并且至少拉开一定间隔。推理时，`nets/facenet.py` 会通过 `F.normalize(..., p=2, dim=1)` 得到单位长度的 embedding，再由预测代码计算两张图片之间的距离。

需要注意的是，本项目主要做“两张人脸是否相似”的验证流程，不包含完整的人脸检测、自动抠脸和身份库检索模块。实际使用时，建议输入已经裁剪好、清晰、正脸、光照稳定的人脸图片，再结合自己的数据确定距离阈值。
