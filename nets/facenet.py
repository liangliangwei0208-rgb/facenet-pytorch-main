...
def load_local_backbone_weights(filename):
    # 预训练主干权重统一从本项目的 model_data 目录读取，避免依赖外部下载链接。
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    weights_path = os.path.join(project_root, "model_data", filename)
    if not os.path.exists(weights_path):
        raise FileNotFoundError("未找到预训练权重文件，请放到 model_data 目录: {}".format(weights_path))
...
class mobilenet(nn.Module):
    def __init__(self, pretrained):
        ...
        if pretrained:
            state_dict = load_local_backbone_weights("backbone_weights_of_mobilenetv1.pth")
...
class inception_resnet(nn.Module):
    def __init__(self, pretrained):
        ...
        if pretrained:
            state_dict = load_local_backbone_weights("backbone_weights_of_inception_resnetv1.pth")
            self.model.load_state_dict(state_dict)
